//===-- NewcpuFrameLowering.cpp - Newcpu Frame Information ---------------====//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Newcpu implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "newcpu-frame-lowering"
...
static cl::opt<bool> MBDisableStackAdjust(
  "disable-newcpu-stack-adjust",
  cl::init(false),
  cl::desc("Disable Newcpu stack layout adjustment."),
  cl::Hidden);
...
static void replaceFrameIndexes(MachineFunction &MF, 
  ...
  for (; FRI != FRE; ++FRI) {
    ...
    NewcpuFI->recordReplacement(FRI->first, NFI);
...
static void analyzeFrameIndexes(MachineFunction &MF) {
  ...
  const SmallVector<int, 16> &LiveInFI = NewcpuFI->getLiveIn();
  ...
  for (unsigned i = 0, e = LiveInFI.size(); i < e; ++i) {
    for (MachineBasicBlock::iterator I=MIB; I != MIE; ++I) {
      if (I->getOpcode() != Newcpu::LWI || I->getNumOperands() != 3 ||
      ...
      for (SI++; SI != MIE; ++SI) {
        if (!SI->getOperand(0).isReg() ||
            ...
            SI->getOpcode() != Newcpu::SWI) continue;
  ...
  for (MachineRegisterInfo::livein_iterator LI = LII; LI != LIE; ++LI) {
    for (MachineBasicBlock::iterator I=MIB; I != MIE; ++I) {
      ...
      if (I->getOpcode() != Newcpu::SWI || I->getNumOperands() != 3 ||
          !I->getOperand(1).isFI() || !I->getOperand(0).isReg() ||
      ...
      if (I->getOperand(0).getReg() == LI->first) {
        ...
        case Newcpu::R5:  FILoc = -4; break;
        case Newcpu::R6:  FILoc = -8; break;
        case Newcpu::R7:  FILoc = -12; break;
        case Newcpu::R8:  FILoc = -16; break;
        case Newcpu::R9:  FILoc = -20; break;
        case Newcpu::R10: FILoc = -24; break;
        }
...
static void interruptFrameLayout(MachineFunction &MF) {
  ...
  MachineFrameInfo *MFI = MF.getFrameInfo();
  const MachineRegisterInfo &MRI = MF.getRegInfo();
  const NewcpuInstrInfo &TII =
    *static_cast<const NewcpuInstrInfo*>(MF.getTarget().getInstrInfo());
  ...
  for (unsigned r = Newcpu::R3; r <= Newcpu::R12; ++r) {
    if (!MRI.isPhysRegUsed(r) && !(isIntr && r == Newcpu::R11)) continue;
    
    int FI = MFI->CreateStackObject(4,4,false,false);
    VFI.push_back(FI);

    BuildMI(MENT, MENTI, ENTDL, TII.get(Newcpu::SWI), r)
  ...
  BuildMI(MENT, MENTI, ENTDL, TII.get(Newcpu::SWI), Newcpu::R17)
    .addFrameIndex(R17FI).addImm(0);
    
  BuildMI(MENT, MENTI, ENTDL, TII.get(Newcpu::SWI), Newcpu::R18)
  ...
  if (isIntr) {
    ...
    BuildMI(MENT, MENTI, ENTDL, TII.get(Newcpu::MFS), Newcpu::R11)
      .addReg(Newcpu::RMSR);
    BuildMI(MENT, MENTI, ENTDL, TII.get(Newcpu::SWI), Newcpu::R11)
      .addFrameIndex(MSRFI).addImm(0);

    BuildMI(MEXT, MEXTI, EXTDL, TII.get(Newcpu::LWI), Newcpu::R11)
      .addFrameIndex(MSRFI).addImm(0);
    BuildMI(MEXT, MEXTI, EXTDL, TII.get(Newcpu::MTS), Newcpu::RMSR)
      .addReg(Newcpu::R11);
  }

  // Build the epilogue LWI for R17, R18
  BuildMI(MEXT, MEXTI, EXTDL, TII.get(Newcpu::LWI), Newcpu::R18)
    .addFrameIndex(R18FI).addImm(0);

  BuildMI(MEXT, MEXTI, EXTDL, TII.get(Newcpu::LWI), Newcpu::R17)
    .addFrameIndex(R17FI).addImm(0);

  // Build the epilogue LWI for R3 - R12 if needed
  for (unsigned r = Newcpu::R12, i = VFI.size(); r >= Newcpu::R3; --r) {
    if (!MRI.isPhysRegUsed(r)) continue;
    BuildMI(MEXT, MEXTI, EXTDL, TII.get(Newcpu::LWI), r)
...
static void determineFrameLayout(MachineFunction &MF) {
  ...
  NewcpuFI->adjustLoadArgsFI(MFI);
  NewcpuFI->adjustStoreVarArgsFI(MFI);
...
int NewcpuFrameLowering::getFrameIndexOffset(const MachineFunction &MF, int FI) 
  const {
  const NewcpuFunctionInfo *NewcpuFI = MF.getInfo<NewcpuFunctionInfo>();
  if (NewcpuFI->hasReplacement(FI))
    FI = NewcpuFI->getReplacement(FI);
...
bool NewcpuFrameLowering::hasFP(const MachineFunction &MF) const {
  const MachineFrameInfo *MFI = MF.getFrameInfo();
  return MF.getTarget().Options.DisableFramePointerElim(MF) ||
         MFI->hasVarSizedObjects();
}

void NewcpuFrameLowering::emitPrologue(MachineFunction &MF) const {
  ...
  int FPOffset = NewcpuFI->getFPStackOffset();
  int RAOffset = NewcpuFI->getRAStackOffset();
...
//      BuildMI(MBB, MBBI, DL, TII.get(Newcpu::STI))
  ...
  BuildMI(MBB, MBBI, DL, TII.get(Newcpu::ADDIK), Newcpu::R1)
      .addReg(Newcpu::R1).addImm(-StackSize);

  // swi  R15, R1, stack_loc
  if (MFI->adjustsStack() || requiresRA) {
    BuildMI(MBB, MBBI, DL, TII.get(Newcpu::SWI))
        .addReg(Newcpu::R15).addReg(Newcpu::R1).addImm(RAOffset);
  }

  if (hasFP(MF)) {
    // swi  R19, R1, stack_loc
    BuildMI(MBB, MBBI, DL, TII.get(Newcpu::SWI))
      .addReg(Newcpu::R19).addReg(Newcpu::R1).addImm(FPOffset);

    // add R19, R1, R0
    BuildMI(MBB, MBBI, DL, TII.get(Newcpu::ADD), Newcpu::R19)
      .addReg(Newcpu::R1).addReg(Newcpu::R0);
  }
  */
}

void NewcpuFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    // add R1, R19, R0
    BuildMI(MBB, MBBI, dl, TII.get(Newcpu::ADD), Newcpu::R1)
      .addReg(Newcpu::R19).addReg(Newcpu::R0);

    // lwi  R19, R1, stack_loc
    BuildMI(MBB, MBBI, dl, TII.get(Newcpu::LWI), Newcpu::R19)
      .addReg(Newcpu::R1).addImm(FPOffset);
  }

  // lwi R15, R1, stack_loc
  if (MFI->adjustsStack() || requiresRA) {
    BuildMI(MBB, MBBI, dl, TII.get(Newcpu::LWI), Newcpu::R15)
      .addReg(Newcpu::R1).addImm(RAOffset);
  }
...
eliminateCallFramePseudoInstr(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      MachineInstr *New;
      if (Old->getOpcode() == Newcpu::ADJCALLSTACKDOWN) {
        New = BuildMI(MF,Old->getDebugLoc(), TII.get(Newcpu::ADDIK),Newcpu::R1)
                .addReg(Newcpu::R1).addImm(-Amount);
      } else {
        assert(Old->getOpcode() == Newcpu::ADJCALLSTACKUP);
        New = BuildMI(MF,Old->getDebugLoc(), TII.get(Newcpu::ADDIK),Newcpu::R1)
                .addReg(Newcpu::R1).addImm(Amount);
      }
...
processFunctionBeforeCalleeSavedScan(MachineFunction &MF,
  ...
  if (MFI->adjustsStack() || requiresRA) {
    NewcpuFI->setRAStackOffset(0);
    MFI->CreateFixedObject(4,0,true);
  }

  if (hasFP(MF)) {
    NewcpuFI->setFPStackOffset(4);
