//===-- NewcpuMachineFunctionInfo.h - Private data --------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares the Newcpu specific subclass of MachineFunctionInfo.
//
//===----------------------------------------------------------------------===//

#ifndef NEWCPU_MACHINE_FUNCTION_INFO_H
#define NEWCPU_MACHINE_FUNCTION_INFO_H
...
/// NewcpuFunctionInfo - This class is derived from MachineFunction private
/// Newcpu target-specific information for each MachineFunction.
class NewcpuFunctionInfo : public MachineFunctionInfo {
  virtual void anchor();
  ...
  /// NewcpuFIHolder - Holds a FrameIndex and it's Stack Pointer Offset
  struct NewcpuFIHolder {

    int FI;
    int SPOffset;

    NewcpuFIHolder(int FrameIndex, int StackPointerOffset)
  ...
  NewcpuFIHolder GPHolder;

  /// On LowerFormalArguments the stack size is unknown, so the Stack
  /// Pointer Offset calculation of "not in register arguments" must be
  /// postponed to emitPrologue.
  SmallVector<NewcpuFIHolder, 16> FnLoadArgs;
  bool HasLoadArgs;
  ...
  // postponed to emitPrologue.
  SmallVector<NewcpuFIHolder, 4> FnStoreVarArgs;
...
public:
  NewcpuFunctionInfo(MachineFunction& MF)
  ...
  void recordLoadArgsFI(int FI, int SPOffset) {
    ...
    FnLoadArgs.push_back(NewcpuFIHolder(FI, SPOffset));
  }

  void recordStoreVarArgsFI(int FI, int SPOffset) {
    if (!HasStoreVarArgs) HasStoreVarArgs=true;
    FnStoreVarArgs.push_back(NewcpuFIHolder(FI, SPOffset));
  }
