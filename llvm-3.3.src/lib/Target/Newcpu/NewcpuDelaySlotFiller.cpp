//===-- DelaySlotFiller.cpp - Newcpu delay slot filler --------------------===//
//
...
static cl::opt<bool> MBDisableDelaySlotFiller(
  "disable-newcpu-delay-filler",
  cl::init(false),
  cl::desc("Disable the Newcpu delay slot filter."),
  cl::Hidden);
...
namespace {
  struct Filler : public MachineFunctionPass {
    ...
    virtual const char *getPassName() const {
      return "Newcpu Delay Slot Filler";
    }
...
static bool hasUnknownSideEffects(MachineBasicBlock::iterator &I) {
  ...
  if (op == Newcpu::ADDK || op == Newcpu::ADDIK ||
      op == Newcpu::ADDC || op == Newcpu::ADDIC ||
      op == Newcpu::ADDKC || op == Newcpu::ADDIKC ||
      op == Newcpu::RSUBK || op == Newcpu::RSUBIK ||
      op == Newcpu::RSUBC || op == Newcpu::RSUBIC ||
      op == Newcpu::RSUBKC || op == Newcpu::RSUBIKC)
    return false;
#endif
  return true;
}
...
#if 0
      if (D == MBB.end())
        BuildMI(MBB, ++J, I->getDebugLoc(), TII->get(Newcpu::NOP));
      else
          MBB.splice(++J, &MBB, D);
#endif
    }
...
/// createNewcpuDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Newcpu MachineFunctions
FunctionPass *llvm::createNewcpuDelaySlotFillerPass(NewcpuTargetMachine &tm) {
