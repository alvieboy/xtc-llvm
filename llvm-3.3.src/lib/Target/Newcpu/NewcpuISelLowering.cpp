//===-- NewcpuISelLowering.cpp - Newcpu DAG Lowering Implementation -------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Newcpu uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "newcpu-lower"
#include "NewcpuISelLowering.h"
#include "NewcpuMachineFunction.h"
#include "NewcpuSubtarget.h"
#include "NewcpuTargetMachine.h"
#include "NewcpuTargetObjectFile.h"
...
static bool CC_Newcpu_AssignReg(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
...
: TargetLowering(TM, new NewcpuTargetObjectFile()) {
  ...
  // Newcpu has no REM or DIVREM operations.
...
//  if (!Subtarget->hasDiv()) {
 ...
 // }
  ...
  // Newcpu doesn't have extending float->double load/store
...
//===----------------------------------------------------------------------===//
MachineBasicBlock*
NewcpuTargetLowering::EmitInstrWithCustomInserter(MachineInstr *MI,
...
switch (MI->getOpcode()) {
  ...
  case Newcpu::Select_FCC:
  case Newcpu::Select_CC:
...
MachineBasicBlock*
NewcpuTargetLowering::EmitCustomShift(MachineInstr *MI,
...
DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" called");
  ...
  unsigned IAMT = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  BuildMI(MBB, dl, TII->get(Newcpu::ANDI), IAMT)
    .addReg(MI->getOperand(2).getReg())
    .addImm(31);

  unsigned IVAL = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  BuildMI(MBB, dl, TII->get(Newcpu::ADDIK), IVAL)
    .addReg(MI->getOperand(1).getReg())
    .addImm(0);

  BuildMI(MBB, dl, TII->get(Newcpu::BEQID))
    .addReg(IAMT)
    .addMBB(finish);

  unsigned DST = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  unsigned NDST = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  BuildMI(loop, dl, TII->get(Newcpu::PHI), DST)
    .addReg(IVAL).addMBB(MBB)
    .addReg(NDST).addMBB(loop);

  unsigned SAMT = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  unsigned NAMT = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  BuildMI(loop, dl, TII->get(Newcpu::PHI), SAMT)
    .addReg(IAMT).addMBB(MBB)
    .addReg(NAMT).addMBB(loop);

  if (MI->getOpcode() == Newcpu::ShiftL)
    BuildMI(loop, dl, TII->get(Newcpu::ADD), NDST).addReg(DST).addReg(DST);
  else if (MI->getOpcode() == Newcpu::ShiftRA)
    BuildMI(loop, dl, TII->get(Newcpu::SRA), NDST).addReg(DST);
  else if (MI->getOpcode() == Newcpu::ShiftRL)
    BuildMI(loop, dl, TII->get(Newcpu::SRL), NDST).addReg(DST);
  else
    llvm_unreachable("Cannot lower unknown shift instruction");

  BuildMI(loop, dl, TII->get(Newcpu::ADDIK), NAMT)
    .addReg(SAMT)
    .addImm(-1);

  BuildMI(loop, dl, TII->get(Newcpu::BNEID))
    .addReg(NAMT)
    .addMBB(loop);

  BuildMI(*finish, finish->begin(), dl,
          TII->get(Newcpu::PHI), MI->getOperand(0).getReg())
    .addReg(IVAL).addMBB(MBB)
    .addReg(NDST).addMBB(loop);
...
MachineBasicBlock*
NewcpuTargetLowering::EmitCustomSelect(MachineInstr *MI,
                                       MachineBasicBlock *MBB) const {
  const TargetInstrInfo *TII = getTargetMachine().getInstrInfo();
  DebugLoc dl = MI->getDebugLoc();
DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" called");
...
#if 0
  case NewcpuCC::EQ: Opc = Newcpu::BEQID; break;
  case NewcpuCC::NE: Opc = Newcpu::BNEID; break;
  case NewcpuCC::GT: Opc = Newcpu::BGTID; break;
  case NewcpuCC::LT: Opc = Newcpu::BLTID; break;
  case NewcpuCC::GE: Opc = Newcpu::BGEID; break;
  case NewcpuCC::LE: Opc = Newcpu::BLEID; break;
#endif
  }
  ...
  //  ...
  //BuildMI(dneBB, dl, TII->get(Newcpu::PHI), MI->getOperand(0).getReg())
...
MachineBasicBlock*
NewcpuTargetLowering::EmitCustomAtomic(MachineInstr *MI,
                                       MachineBasicBlock *MBB) const {
  const TargetInstrInfo *TII = getTargetMachine().getInstrInfo();
  DebugLoc dl = MI->getDebugLoc();
DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" called");
  ...
  BuildMI(start, dl, TII->get(Newcpu::LWX), MI->getOperand(0).getReg())
    .addReg(MI->getOperand(1).getReg())
    .addReg(Newcpu::R0);
  ...
  case Newcpu::LAA32: {
    ...
    case Newcpu::LAA32: opcode = Newcpu::ADDIK; break;
    case Newcpu::LAS32: opcode = Newcpu::RSUBIK; break;
    case Newcpu::LAD32: opcode = Newcpu::AND; break;
    case Newcpu::LAO32: opcode = Newcpu::OR; break;
    case Newcpu::LAX32: opcode = Newcpu::XOR; break;
    case Newcpu::LAN32: opcode = Newcpu::AND; break;
    }

    finalReg = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
    start->addSuccessor(exit);
    start->addSuccessor(start);
    ...
    if (MI->getOpcode() == Newcpu::LAN32) {
      unsigned tmp = finalReg;
      finalReg = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
      BuildMI(start, dl, TII->get(Newcpu::XORI), finalReg)
        .addReg(tmp)
        .addImm(-1);
    }
    break;
  ...
  case Newcpu::CAS32: {
    ...
    unsigned CMP = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
    BuildMI(start, dl, TII->get(Newcpu::CMP), CMP)
      .addReg(MI->getOperand(0).getReg())
      .addReg(MI->getOperand(2).getReg());

    BuildMI(start, dl, TII->get(Newcpu::BNEID))
      .addReg(CMP)
      .addMBB(exit);
  ...
  unsigned CHK = R.createVirtualRegister(&Newcpu::GPRegsRegClass);
  BuildMI(final, dl, TII->get(Newcpu::SWX))
    .addReg(finalReg)
    .addReg(MI->getOperand(1).getReg())
    .addReg(Newcpu::R0);

  BuildMI(final, dl, TII->get(Newcpu::ADDIC), CHK)
    .addReg(Newcpu::R0)
    .addImm(0);

  BuildMI(final, dl, TII->get(Newcpu::BNEID))
    .addReg(CHK)
    .addMBB(start);
...
SDValue NewcpuTargetLowering::LowerSELECT_CC(SDValue Op,
                                             SelectionDAG &DAG) const {
...
LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(NewcpuISD::Wrap, dl, MVT::i32, GA);
}
...
LowerJumpTable(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(NewcpuISD::Wrap, dl, MVT::i32, JTI);
}
...
LowerConstantPool(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue CP = DAG.getTargetConstantPool(C, MVT::i32, N->getAlignment(),
                                         N->getOffset(), 0);
  return DAG.getNode(NewcpuISD::Wrap, dl, MVT::i32, CP);
}

SDValue NewcpuTargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  NewcpuFunctionInfo *FuncInfo = MF.getInfo<NewcpuFunctionInfo>();
...
#include "NewcpuGenCallingConv.inc"
...
LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Newcpu does not yet support tail call optimization
  isTailCall = false;

  // The Newcpu requires stack slots for arguments passed to var arg
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_Newcpu);
  ...
  // NewcpuJmpLink = #chain, #target_address, #opt_in_flags...
  ...
  Chain  = DAG.getNode(NewcpuISD::JmpLink, dl, NodeTys, &Ops[0], Ops.size());
  InFlag = Chain.getValue(1);
...
LowerCallResult(SDValue Chain, SDValue InFlag, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_Newcpu);
...
LowerFormalArguments(SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
  ...
  NewcpuFI->setVarArgsFrameIndex(0);
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_Newcpu);
  SDValue StackPtr;
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      NewcpuFI->recordLoadArgsFI(FI, -StackLoc);
      NewcpuFI->recordLiveIn(FI);
  ...
  if ((isVarArg) && ArgRegEnd) {
    ...
    // The last register argument that must be saved is Newcpu::R10
    const TargetRegisterClass *RC = &Newcpu::GPRegsRegClass;

    unsigned Begin = getNewcpuRegisterNumbering(Newcpu::R5);
    unsigned Start = getNewcpuRegisterNumbering(ArgRegEnd+1);
    unsigned End   = getNewcpuRegisterNumbering(Newcpu::R10);
    unsigned StackLoc = Start - Begin + 1;

    for (; Start <= End; ++Start, ++StackLoc) {
      unsigned Reg = getNewcpuRegisterFromNumbering(Start);
      unsigned LiveReg = MF.addLiveIn(Reg, RC);
      SDValue ArgValue = DAG.getCopyFromReg(Chain, dl, LiveReg, MVT::i32);

      int FI = MFI->CreateFixedObject(4, 0, true);
      NewcpuFI->recordStoreVarArgsFI(FI, -(StackLoc*4));
      SDValue PtrOff = DAG.getFrameIndex(FI, getPointerTy());
      ...
      if (!NewcpuFI->getVarArgsFrameIndex())
        NewcpuFI->setVarArgsFrameIndex(FI);
    }
  }
...
LowerReturn(SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Newcpu);
...
#if 0
  unsigned Ret = (CallConv == CallingConv::MBLAZE_INTR) ? NewcpuISD::IRet
                                                        : NewcpuISD::Ret;
...
//===----------------------------------------------------------------------===//
//                           Newcpu Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
NewcpuTargetLowering::ConstraintType NewcpuTargetLowering::
getConstraintType(const std::string &Constraint) const
{
  // Newcpu specific constrainy
  //
...
NewcpuTargetLowering::getSingleConstraintMatchWeight(
...
/// pointer.
std::pair<unsigned, const TargetRegisterClass*> NewcpuTargetLowering::
getRegForInlineAsmConstraint(const std::string &Constraint, EVT VT) const {
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
      return std::make_pair(0U, &Newcpu::GPRegsRegClass);
...
bool NewcpuTargetLowering::
isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The Newcpu target isn't yet aware of offsets.
  return false;
}
