//===-- XTCAsmPrinter.cpp - XTC LLVM assembly writer ----------------===//
//
...
// of machine-dependent LLVM code to GAS-format XTC assembly language.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "xtc-asm-printer"
...
namespace {
  class XTCAsmPrinter : public AsmPrinter {
    const XTCSubtarget *Subtarget;
  public:
    explicit XTCAsmPrinter(TargetMachine &TM, MCStreamer &Streamer)
      : AsmPrinter(TM, Streamer) {
      Subtarget = &TM.getSubtarget<XTCSubtarget>();
    }

    virtual const char *getPassName() const {
      return "XTC Assembly Printer";
    }
...
void XTCAsmPrinter::printSavedRegsBitmask() {
  ...
  for (unsigned i = 0, e = CSI.size(); i != e; ++i) {
    ...
    unsigned RegNum = getXTCRegisterNumbering(Reg);
    if (XTC::GPRegsRegClass.contains(Reg))
      CPUBitmask |= (1 << RegNum);
  }

  // Return Address and Frame registers must also be set in CPUBitmask.
  if (TFI->hasFP(*MF))
    CPUBitmask |= (1 <<  getXTCRegisterNumbering(RI.getFrameRegister(*MF)));

  if (MFI->adjustsStack())
    CPUBitmask |= (1 << getXTCRegisterNumbering(RI.getRARegister()));
...
/// Frame Directive
void XTCAsmPrinter::emitFrameDirective() {
  ...
  OutStreamer.EmitRawText("\t.frame\t" +
                          Twine(XTCInstPrinter::getRegisterName(stkReg)) +
                          "," + Twine(stkSze) + "," +
                          Twine(XTCInstPrinter::getRegisterName(retReg)));
}
...
void XTCAsmPrinter::EmitFunctionBodyStart() {
  if (!OutStreamer.hasRawTextSupport())
    return;
...
//===----------------------------------------------------------------------===//
void XTCAsmPrinter::EmitInstruction(const MachineInstr *MI) {
  XTCMCInstLower MCInstLowering(OutContext, *this);
...
void XTCAsmPrinter::printOperand(const MachineInstr *MI, int opNum,
                                    raw_ostream &O) {
  const MachineOperand &MO = MI->getOperand(opNum);

  switch (MO.getType()) {
  case MachineOperand::MO_Register:
    O << XTCInstPrinter::getRegisterName(MO.getReg());
    break;
...
void XTCAsmPrinter::printUnsignedImm(const MachineInstr *MI, int opNum,
                                        raw_ostream &O) {
  const MachineOperand &MO = MI->getOperand(opNum);
  if (MO.isImm())
...
void XTCAsmPrinter::printFSLImm(const MachineInstr *MI, int opNum,
                                   raw_ostream &O) {
  const MachineOperand &MO = MI->getOperand(opNum);
  if (MO.isImm())
