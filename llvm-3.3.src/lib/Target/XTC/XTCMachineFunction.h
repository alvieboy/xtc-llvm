//===-- XTCMachineFunctionInfo.h - Private data --------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares the XTC specific subclass of MachineFunctionInfo.
//
//===----------------------------------------------------------------------===//

#ifndef XTC_MACHINE_FUNCTION_INFO_H
#define XTC_MACHINE_FUNCTION_INFO_H
...
/// XTCFunctionInfo - This class is derived from MachineFunction private
/// XTC target-specific information for each MachineFunction.
class XTCFunctionInfo : public MachineFunctionInfo {
  virtual void anchor();
  ...
  /// XTCFIHolder - Holds a FrameIndex and it's Stack Pointer Offset
  struct XTCFIHolder {

    int FI;
    int SPOffset;

    XTCFIHolder(int FrameIndex, int StackPointerOffset)
  ...
  XTCFIHolder GPHolder;

  /// On LowerFormalArguments the stack size is unknown, so the Stack
  /// Pointer Offset calculation of "not in register arguments" must be
  /// postponed to emitPrologue.
  SmallVector<XTCFIHolder, 16> FnLoadArgs;
  bool HasLoadArgs;
  ...
  // postponed to emitPrologue.
  SmallVector<XTCFIHolder, 4> FnStoreVarArgs;
...
public:
  XTCFunctionInfo(MachineFunction& MF)
  ...
  void recordLoadArgsFI(int FI, int SPOffset) {
    ...
    FnLoadArgs.push_back(XTCFIHolder(FI, SPOffset));
  }

  void recordStoreVarArgsFI(int FI, int SPOffset) {
    if (!HasStoreVarArgs) HasStoreVarArgs=true;
    FnStoreVarArgs.push_back(XTCFIHolder(FI, SPOffset));
  }
