//===-- DelaySlotFiller.cpp - XTC delay slot filler --------------------===//
//
...
static cl::opt<bool> MBDisableDelaySlotFiller(
  "disable-xtc-delay-filler",
  cl::init(false),
  cl::desc("Disable the XTC delay slot filter."),
  cl::Hidden);
...
static bool hasUnknownSideEffects(MachineBasicBlock::iterator &I) {
  ...
  if (op == XTC::ADDK || op == XTC::ADDIK ||
      op == XTC::ADDC || op == XTC::ADDIC ||
      op == XTC::ADDKC || op == XTC::ADDIKC ||
      op == XTC::RSUBK || op == XTC::RSUBIK ||
      op == XTC::RSUBC || op == XTC::RSUBIC ||
...
/// slots in XTC MachineFunctions
FunctionPass *llvm::createXTCDelaySlotFillerPass(XTCTargetMachine &tm) {
