...
bool DAGCombiner::CombineToPreIndexedLoadStore(SDNode *N) {
  ...
  if ((Ptr.getOpcode() != ISD::ADD && Ptr.getOpcode() != ISD::SUB) ||
      ...
      DEBUG(dbgs()<<"No add, sub, or has only one use, opcode "<<Instruction::getOpcodeName(Ptr.getOpcode())<<"\n");
  ...
  if (!TLI.getPreIndexedAddressParts(N, BasePtr, Offset, AM, DAG)) {
      DEBUG(dbgs()<<"Not accepted\n");

      return false;
  }

  DEBUG(dbgs()<<"getPreIndexedAddressParts accepted\n");
