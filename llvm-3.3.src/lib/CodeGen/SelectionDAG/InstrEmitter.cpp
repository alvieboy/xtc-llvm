...
static unsigned countOperands(SDNode *Node, unsigned NumExpUses,
  ...
  while (N && Node->getOperand(N - 1).getValueType() == MVT::Glue) {
      fprintf(stderr,"Skipping glue at index %d\n", N);
      --N;
  }
  if (N && Node->getOperand(N - 1).getValueType() == MVT::Other) {
      fprintf(stderr,"Skipping MVT::Other at index %d\n", N);
      --N; // Ignore chain if it exists.
  }

  // Count RegisterSDNode and RegisterMaskSDNode operands for NumImpUses.
  fprintf(stderr,"Setting numimpuses to %d - %d <- expuses\n",N,NumExpUses);
  ...
  for (unsigned I = N; I > NumExpUses; --I) {
    ...
    if (RegisterSDNode *RN = dyn_cast<RegisterSDNode>(Node->getOperand(I - 1)))
      if (TargetRegisterInfo::isPhysicalRegister(RN->getReg()))
          continue;
    fprintf(stderr,"Setting numimpuses to %d - %d\n",N,I);
    NumImpUses = N - I;
    break;
  }
...
EmitMachineNode(SDNode *Node, bool IsClone, bool IsCloned,
  ...
  if (II.isVariadic())
    assert(NumMIOperands >= II.getNumOperands() &&
      ...
      Node->dump();
      DEBUG(dbgs()<<"\n");
      fprintf(stderr,"NumMIOperands is %d\n",NumMIOperands);
      fprintf(stderr,"II1 is %d\n", II.getNumOperands());
      fprintf(stderr,"II2 is %d, implicit %d, impuses %d\n",    (II.getNumOperands() + II.getNumImplicitDefs() + NumImpUses),
