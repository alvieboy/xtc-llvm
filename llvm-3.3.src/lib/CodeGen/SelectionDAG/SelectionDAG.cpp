...
SDNode *SelectionDAG::MorphNodeTo(SDNode *N, unsigned Opc,
  ...
  DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t1\n");
  DEBUG(dbgs()<<"Num ops" <<NumOps);
  DEBUG(N->dump());
  ...
  DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t2\n");
  ...
  for (SDNode::op_iterator I = N->op_begin(), E = N->op_end(); I != E; ) {
    ...
    DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3\n");

  if (MachineSDNode *MN = dyn_cast<MachineSDNode>(N)) {
      // Initialize the memory references information.
      DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3.1\n");

      MN->setMemRefs(0, 0);
        DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3.2\n");
    ...
    if (NumOps > MN->NumOperands || !MN->OperandsNeedDelete) {
      if (MN->OperandsNeedDelete)
        ...
        DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3.3\n");

        if (NumOps > array_lengthof(MN->LocalOperands)) {
        // We're creating a final node that will live unmorphed for the
        // remainder of the current SelectionDAG iteration, so we can allocate
          // the operands directly out of a pool with no recycling metadata.
            DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3.4\n");

        MN->InitOperands(OperandAllocator.Allocate<SDUse>(NumOps),
                         Ops, NumOps);    }
        else {
            DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3.5\n");
            DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" MN "<<(void*)MN<<"\n");
            int z;
            for (z=0;z<NumOps; z++) {
                DEBUG(dbgs()<<"Dumping op number "<<z<<"\n");
                Ops[z].dump();
            }
      ...
      MN->OperandsNeedDelete = false;
          ...
          DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t3.6\n");
    ...
    DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t4\n");
  ...
  if (!DeadNodeSet.empty()) {
    ...
    DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" t5\n");
...
void SelectionDAG::ReplaceAllUsesWith(SDNode *From, SDNode *To) {
    ...
    for (unsigned i = 0, e = From->getNumValues(); i != e; ++i) {
        DEBUG(dbgs()<<"Index "<<i<<":\n");
        DEBUG(dbgs()<<"From: "<<From->getValueType(i).getSimpleVT().SimpleTy<<"\n");
        DEBUG(dbgs()<<"To: "<<To->getValueType(i).getSimpleVT().SimpleTy<<"\n");
