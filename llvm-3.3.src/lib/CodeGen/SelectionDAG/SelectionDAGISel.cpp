...
SelectCodeCommon(SDNode *NodeToMatch, const unsigned char *MatcherTable,
  ...
  SDNode *Parent = 0;
    ...
    case OPC_EmitInteger: {
      ...
      if (Val & 128)
        Val = GetVBR(Val, MatcherTable, MatcherIndex);
      DEBUG(dbgs() << "Recording node int\n"); CurDAG->getTargetConstant(Val, VT)->dump();
    ...
    case OPC_EmitRegister: {
      ...
      DEBUG(dbgs() << "Recording node reg\n");  CurDAG->getRegister(RegNo, VT)->dump();
      RecordedNodes.push_back(std::pair<SDValue, SDNode*>(
    ...
    case OPC_EmitRegister2: {
      ...
      DEBUG(dbgs() << "Recording node reg2\n");  CurDAG->getRegister(RegNo, VT)->dump();
    ...
    case OPC_EmitMergeInputChains1_1: {  // OPC_EmitMergeInputChains, 1, 1
      ...
      DEBUG(dbgs()<<"Before handlemergeinputchains " <<RecordedNodes.size()<<"\n");
      // Merge the input chains if they are not intra-pattern references.
      InputChain = HandleMergeInputChains(ChainNodesMatched, CurDAG);
      DEBUG(dbgs()<<"After handlemergeinputchains " <<RecordedNodes.size()<<"\n");
    ...
    case OPC_EmitNodeXForm: {
      ...
      DEBUG(dbgs() << "Recording node xform\n");  Res->dump();
    ...
    case OPC_MorphNodeTo: {
      ...
      for (unsigned i = 0; i != NumOps; ++i) {
        ...
        DEBUG(dbgs()<<"Here(i): size is "<<Ops.size()<<" recno "<<RecNo<<" recorded "<<RecordedNodes.size()<<"\n");
        RecordedNodes[RecNo].first->dump();
        Ops.push_back(RecordedNodes[RecNo].first);
      }
      DEBUG(dbgs()<<"Here-1: size is "<<Ops.size()<<"\n");
      ...
      DEBUG(dbgs()<<"Here0: size is "<<Ops.size()<<"\n");
      ...
      DEBUG(dbgs()<<"Here1: size is "<<Ops.size()<<"\n");
      // Create the node.
      SDNode *Res = 0;
      if (Opcode != OPC_MorphNodeTo) {
        ...
        for (unsigned i = 0, e = VTs.size(); i != e; ++i) {
          ...
          DEBUG(dbgs() << "Recording node from machine\n");  Res->dump();
          RecordedNodes.push_back(std::pair<SDValue,SDNode*>(SDValue(Res, i),
                                                             (SDNode*) 0));
        }

      } else if (NodeToMatch->getOpcode() != ISD::DELETED_NODE) {
          DEBUG(dbgs()<<__PRETTY_FUNCTION__<<" deleted node!\n");
